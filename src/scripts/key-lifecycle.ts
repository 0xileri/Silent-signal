...
step(`paid call (${MODEL})`, `"${reply.text}" · ${reply.usage.prompt_tokens}+${reply.usage.completion_tokens} tokens · ${usd(metered)} at list price${typeof reply.usage.cost === 'number' ? ` · gateway reports ${usd(reply.usage.cost)}` : ''}`)
